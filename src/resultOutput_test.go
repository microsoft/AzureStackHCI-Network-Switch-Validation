...
var (
	inputVariables = &InputType{InterfaceGUID: "\\Device\\NPF_{0217D729-CED0-4D06-9C66-592E032A37A8}", InterfaceAlias: "Ethernet", NativeVlanID: 710, AllVlanIDs: []int{1, 710, 711, 712}, MTUSize: 9214, ETSMaxClass: 8, ETSBWbyPG: "0:48,1:0,2:0,3:50,4:0,5:2,6:0,7:0", PFCMaxClass: 8, PFCPriorityEnabled: "0:0,1:0,2:0,3:1,4:0,5:0,6:0,7:0"}
)
...
func TestResultOutput(t *testing.T) {
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			outputPdfFile := filepath.Join(testOutputFolder, tc.inputFileName+".pdf")
			got.outputPDFFile(outputPdfFile)
			outputYamlFile := filepath.Join(testOutputFolder, tc.inputFileName+".yml")
